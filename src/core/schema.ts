import { z } from "zod";
import type { ShapeKeys, FieldPaths } from "./types.js";
...
export function describedEnum<const V extends Record<string, string>>(
  ...
  const values = Object.keys(variants) as [
  ...
  const composedDescription = values
    .map((v) => `- "${v}": ${variants[v]}`)
    .join("\n");
 ...
 */
export function composeFieldDescriptions(
