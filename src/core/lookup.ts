 ...
 * });
 */
export interface TypedLookup<IDs extends readonly string[], Keys extends string> {
...
export function defineLookup<const IDs extends readonly string[], const Keys extends string>(
	validIds: IDs,
			...
			for (const [key, value] of Object.entries(table)) {
				...
				for (const v of values) {
					if (!idSet.has(v as IDs[number])) {
						throw new Error(`Lookup key "${key}" references invalid ID "${v}"`);
					}
				}
			}
		},
