...
import type { ExtractPlaceholders, ShapeKeys, ToolDefinition } from "./types.js";

// ─── Typed Prompt Template ───

/** Return type of definePrompt. */
export interface TypedPrompt<Template extends string, Vars extends Record<string, z.ZodType>> {
	...
	return {
		...
		render(values) {
			...
			for (const [key, schema] of Object.entries(config.variables)) {
				const parsed = (schema as z.ZodType).parse((values as Record<string, unknown>)[key]);
...
export interface SchemaPrompt<Template extends string, Shape extends z.ZodRawShape> {
	readonly name: string;
	readonly template: Template;
	render(
		values: {
			[K in ExtractPlaceholders<Template>]: K extends keyof Shape ? z.infer<Shape[K]> : never;
		},
	): string;
}
 ...
 */
export function defineSchemaPrompt<Shape extends z.ZodRawShape, const Template extends string>(
	...
	return {
		...
		// biome-ignore lint/suspicious/noExplicitAny: conditional return type requires runtime cast
	} as any;
}
...
export interface SystemPromptHelpers<Tools extends Record<string, ToolDefinition>> {
 ...
 * `);
 */
export function defineSystemPrompt<Tools extends Record<string, ToolDefinition>>(
